...
CONST(tpl_appmode_mask, OS_CONST) tpl_alarm_app_mode[ALARM_COUNT] = {
  1 /* alarm alertaMensagem1 : stdAppmode */ ,
  1 /* alarm alertaMensagem2 : stdAppmode */ ,
  1 /* alarm alertaMensagem3 : stdAppmode */ 
};
...
/* Task M1 identifier */
#define M1_id 0
CONST(TaskType, AUTOMATIC) M1 = M1_id;

/* Task M2 identifier */
#define M2_id 1
CONST(TaskType, AUTOMATIC) M2 = M2_id;

/* Task M3 identifier */
#define M3_id 2
CONST(TaskType, AUTOMATIC) M3 = M3_id;

/*=============================================================================
 * Declaration of Alarm IDs
 */

/* Alarm alertaMensagem1 identifier */
#define alertaMensagem1_id 0
CONST(AlarmType, AUTOMATIC) alertaMensagem1 = alertaMensagem1_id;

/* Alarm alertaMensagem2 identifier */
#define alertaMensagem2_id 1
CONST(AlarmType, AUTOMATIC) alertaMensagem2 = alertaMensagem2_id;

/* Alarm alertaMensagem3 identifier */
#define alertaMensagem3_id 2
CONST(AlarmType, AUTOMATIC) alertaMensagem3 = alertaMensagem3_id;
 ...
 * Task M1 descriptor
 */
#define APP_Task_M1_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task M1 function prototype
 */

FUNC(void, OS_APPL_CODE) M1_function(void);
#define APP_Task_M1_STOP_SEC_CODE
 ...
 *
 */
#define APP_Task_M1_START_SEC_STACK
#include "tpl_memmap.h"
tpl_stack_word M1_stack_zone[128/sizeof(tpl_stack_word)];
#define APP_Task_M1_STOP_SEC_STACK
#include "tpl_memmap.h"

#define OS_START_SEC_VAR_UNSPECIFIED
#include "tpl_memmap.h"

#define M1_STACK { M1_stack_zone, 128 }

/*
 * Task M1 context
 */
avr_context M1_int_context;
#define M1_CONTEXT &M1_int_context
...
/*
 * Static descriptor of task M1
 */
CONST(tpl_proc_static, OS_CONST) M1_task_stat_desc = {
  /* context                  */  M1_CONTEXT,
  /* stack                    */  M1_STACK,
  /* entry point (function)   */  M1_function,
  /* internal ressource       */  NULL,
  /* task id                  */  M1_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task M1
 */
VAR(tpl_proc, OS_VAR) M1_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
 ...
 * Task M2 descriptor
 */
#define APP_Task_M2_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task M2 function prototype
 */

FUNC(void, OS_APPL_CODE) M2_function(void);
#define APP_Task_M2_STOP_SEC_CODE
 ...
 *
 */
#define APP_Task_M2_START_SEC_STACK
#include "tpl_memmap.h"
tpl_stack_word M2_stack_zone[128/sizeof(tpl_stack_word)];
#define APP_Task_M2_STOP_SEC_STACK
#include "tpl_memmap.h"

#define OS_START_SEC_VAR_UNSPECIFIED
#include "tpl_memmap.h"

#define M2_STACK { M2_stack_zone, 128 }

/*
 * Task M2 context
 */
avr_context M2_int_context;
#define M2_CONTEXT &M2_int_context
...
/*
 * Static descriptor of task M2
 */
CONST(tpl_proc_static, OS_CONST) M2_task_stat_desc = {
  /* context                  */  M2_CONTEXT,
  /* stack                    */  M2_STACK,
  /* entry point (function)   */  M2_function,
  /* internal ressource       */  NULL,
  /* task id                  */  M2_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task M2
 */
VAR(tpl_proc, OS_VAR) M2_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
 ...
 * Task M3 descriptor
 */
#define APP_Task_M3_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task M3 function prototype
 */

FUNC(void, OS_APPL_CODE) M3_function(void);
#define APP_Task_M3_STOP_SEC_CODE
 ...
 *
 */
#define APP_Task_M3_START_SEC_STACK
#include "tpl_memmap.h"
tpl_stack_word M3_stack_zone[128/sizeof(tpl_stack_word)];
#define APP_Task_M3_STOP_SEC_STACK
#include "tpl_memmap.h"

#define OS_START_SEC_VAR_UNSPECIFIED
#include "tpl_memmap.h"

#define M3_STACK { M3_stack_zone, 128 }

/*
 * Task M3 context
 */
avr_context M3_int_context;
#define M3_CONTEXT &M3_int_context
...
/*
 * Static descriptor of task M3
 */
CONST(tpl_proc_static, OS_CONST) M3_task_stat_desc = {
  /* context                  */  M3_CONTEXT,
  /* stack                    */  M3_STACK,
  /* entry point (function)   */  M3_function,
  /* internal ressource       */  NULL,
  /* task id                  */  M3_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task M3
 */
VAR(tpl_proc, OS_VAR) M3_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
 ...
 */
/*-----------------------------------------------------------------------------
 * Alarm alertaMensagem1 descriptor
 *
 * This alarm does the activation of task M1
 */
...
CONST(tpl_task_activation_action, OS_CONST) alertaMensagem1_action = {
  {
...
CONST(tpl_alarm_static, OS_CONST) alertaMensagem1_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_ID == YES)
    /* id of the alarm for tracing  */  , alertaMensagem1_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&alertaMensagem1_action
};
...
VAR(tpl_time_obj, OS_VAR) alertaMensagem1_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&alertaMensagem1_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  100,
    /* date                         */  100,
...
/*-----------------------------------------------------------------------------
 * Alarm alertaMensagem2 descriptor
 *
 * This alarm does the activation of task M2
 */
...
CONST(tpl_task_activation_action, OS_CONST) alertaMensagem2_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  M2_id
...
CONST(tpl_alarm_static, OS_CONST) alertaMensagem2_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_ID == YES)
    /* id of the alarm for tracing  */  , alertaMensagem2_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&alertaMensagem2_action
};
...
VAR(tpl_time_obj, OS_VAR) alertaMensagem2_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&alertaMensagem2_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  100,
    /* date                         */  100,
...
/*-----------------------------------------------------------------------------
 * Alarm alertaMensagem3 descriptor
 *
 * This alarm does the activation of task M3
 */
...
CONST(tpl_task_activation_action, OS_CONST) alertaMensagem3_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  M3_id
...
CONST(tpl_alarm_static, OS_CONST) alertaMensagem3_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_ID == YES)
    /* id of the alarm for tracing  */  , alertaMensagem3_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&alertaMensagem3_action
};
...
VAR(tpl_time_obj, OS_VAR) alertaMensagem3_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&alertaMensagem3_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  250,
    /* date                         */  250,
...
CONSTP2VAR(tpl_time_obj, AUTOMATIC, OS_APPL_DATA)
  ...
  &alertaMensagem1_alarm_desc,
  &alertaMensagem2_alarm_desc,
  &alertaMensagem3_alarm_desc
};
#define OS_STOP_SEC_CONST_UNSPECIFIED
#include "tpl_memmap.h"
